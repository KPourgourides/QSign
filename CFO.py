"""
CFO Client for a multi-party quantum signature protocol using SimulaQron.

This module connects the CFO executive to the QSign server, 
...
async def run_client(reader, writer) -> None:
    """
    ...
    Returns:
        None
    """
    print(f"CFO: Connection succesful", flush=True)

    epr_socket = EPRSocket("QSign")
    conn = NetQASMConnection("CFO", epr_sockets=[epr_socket], max_qubits=1000)
    await run_signature_request(reader, writer, conn, epr_socket)


async def run_signature_request(reader, writer, conn, epr_socket) -> None:
    """
    ...
    print(f"CFO: Received point x={x}, f(x)={y} from QSign")
    ...
    print(f"CFO: Received corrections from QSign")
    #----------------------------------------------
    # Adding secret and random phases
    c = (y*(1/(1-x))*((3)/(3-x))) % p
    print(f'CFO: Calculated c={c}')

    phase_secret = 2*np.pi*c/p
    phase_random = np.random.choice([0, 0.5, 1, 1.5])

    if SIGN:
        print("CFO: Signing the contract...")
    else:
        print("CFO: Decided not to sign")
    ...
    await writer.drain()
    print(f"CFO: Sent corrections to QSign")
    ...
    if request == "PHASE?":
        ...
        await writer.drain()
    print(f"CFO: Sent chosen phase φ={phase_random}π to QSign")

    # Receive final verdict
    verdict = (await reader.readline()).decode().strip()
    if verdict.startswith("NO"):
        print('CFO: Received final decision -> Contract not signed!')
    else:
        print('CFO: Received final decision -> Contract signed!')
    conn.close()
    

def _parse_argv(argv: list[str]):
    """
    Parses command-line arguments to determine CFO signing decision.
    ...
    Returns:
        SIGN : bool
            True if the CFO chooses to sign the contract, False otherwise.
    """
    if len(argv) != 2:
        print("Usage: python3 CFO.py SIGN or NO_SIGN")
        sys.exit(1)
    ...
    if msg=='SIGN':
        ...
        "Usage: python3 CFO.py SIGN or NO_SIGN")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    print(f"CFO: Connecting to QSign...", flush=True)
