"""
CEO Client for a multi-party quantum signature protocol using SimulaQron.

This module connects the CEO executive to the QSign server, 
...
async def run_client(reader: StreamReader, writer: StreamWriter) -> None:
    ...
    Returns:
        None
    """
    print(f"CEO: Connection succesful", flush=True)

    epr_socket = EPRSocket("QSign")
    conn = NetQASMConnection("CEO", epr_sockets=[epr_socket], max_qubits=1000)
...
async def run_signature_request(reader: StreamReader, writer: StreamWriter,
    ...
    print(f"CEO: Received point x={x}, f(x)={y} from QSign")
    qlist = []
    ...
    print(f"CEO: Received corrections from QSign")
    #----------------------------------------------
    # Adding secret and random phases
    c = (y*(2/(2-x))*((3)/(3-x))) % p
    print(f'CEO: Calculated c={c}')

    phase_secret = 2*np.pi*c/p
    phase_random = np.random.choice([0, 0.5, 1, 1.5])

    if SIGN:
        print("CEO: Signing the contract...")
    else:
        print("CEO: Decided not to sign")
    ...
    await writer.drain()
    print(f"CEO: Sent corrections to QSign")
    ...
    if request == "PHASE?":
        ...
        await writer.drain()
    print(f"CEO: Sent chosen phase φ={phase_random}π to QSign")
    
    # Receive final verdict
    verdict = (await reader.readline()).decode().strip()
    if verdict.startswith("NO"):
        print('CEO: Received final decision -> Contract not signed!')
    else:
        print('CEO: Received final decision -> Contract signed!')
    conn.close()

    
def _parse_argv(argv: list[str]):
    """
    Parses command-line arguments to determine CEO signing decision.
    ...
    Returns:
        SIGN : bool
            True if the CEO chooses to sign the contract, False otherwise.
    """
    if len(argv) != 2:
        print("Usage: python3 CEO.py SIGN or NO_SIGN")
        sys.exit(1)
    ...
    if msg=='SIGN':
        ...
        "Usage: python3 CEO.py SIGN or NO_SIGN")
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    print(f"CEO: Connecting to QSign...", flush=True)
